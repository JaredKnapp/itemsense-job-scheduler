...
/**
 * A bean to contain a Quartz Trigger and Quartz Job, so I can display them together.
 * Yes, Jobs can have more than one trigger, but this bean is to load a table of triggers.
 *
 * @author Paul Hill
 */
...
public class TriggeredJob {
    ...
    public TriggeredJob(CronTriggerImpl trigger, JobDetail jobDetail) {
        ...
        ItemSenseConfig ItemSenseConfigJob = (ItemSenseConfig) jobDetail.getJobDataMap().get(ConnectorConstants.JOB_DATA_MAP_ITEMSENSE_CONFIG);
        //ConnectorConfig sharedConfig = (ConnectorConfig)jobDetail.getJobDataMap().get(ConnectorConstants.JOB_DATA_MAP_SHARED_CONFIG);
